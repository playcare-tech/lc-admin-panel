...
export async function onRequest(context) {
  ...
  try {
    ...
    if (context.request.method === "GET") {
      const review = await getLivechatAgentQaReview(context.env, id);
      if (!review) return errorResponse("LiveChat agent QA review not found.", 404);
      return json(review);
    }

    const body = await readJson(context.request);
    if (body.action === "process" || body.action === "retry") {
      return json(await processLivechatAgentQaReview(context.env, id, { force: body.action === "retry" || Boolean(body.force) }));
    }

    const result = await decideLivechatAgentQaReview(context.env, id, {
      ...body,
      reviewer: auth.session.user,
    });
    if (!result.decided && result.reason === "not_found") {
      return errorResponse("LiveChat agent QA review not found.", 404);
    }
    if (!result.decided) {
      return errorResponse("Unsupported LiveChat agent QA review decision.", 400, { reason: result.reason });
    }
    return json(result);
  } catch (error) {
    ...
    return serverErrorResponse(error, "Failed to handle LiveChat agent QA review.");
  }
}
